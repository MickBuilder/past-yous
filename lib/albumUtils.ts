...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.fillStyle = '#2d2c2e'; // Dark background from theme: oklch(0.1797 0.0043 308.1928)
    ctx.fillRect(0, 0, canvasWidth, canvasHeight);

    // 2. Draw the title
    ctx.fillStyle = '#cfcfcf'; // Light foreground from theme
    ...
    reversedImages.forEach(({ decade, img }, reversedIndex) => {
        ...
        // Draw the polaroid frame (centered at the new origin)
        ctx.fillStyle = '#2e2e2e'; // Dark card color from theme
        ...
        const imageContainerX = -imageContainerWidth / 2;
        
        const imgX = imageContainerX + (imageContainerWidth - drawWidth) / 2;
        ...
        ctx.fillStyle = '#cfcfcf'; // Light text for dark card
